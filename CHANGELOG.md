...
### Added
- `examples/cairo-basics.f90` now also saves the drawing in a SVG file.
...
- `src/cfwrapper/`: added a `-s` option to add a suffix to the functions names.
- `src/cfwrapper/`: added a `-l` option to add the directories containing the header files and a `-m` option to add the corresponding Fortran modules.
...
### Fixed
- `examples/hl_cairo_viewer.f90`: the image is now redrawn when the window is resized. And the "Next>" button now becomes sensitive when a second file is added.
