...
! GDK_AVAILABLE_IN_ALL
!void gdk_wayland_device_pad_set_feedback (GdkDevice *device, GdkDevicePadFeature element, guint idx, const char *label);
...
! GDK_AVAILABLE_IN_ALL
!void gdk_wayland_surface_set_application_id (GdkSurface *surface, const char *application_id);
subroutine gdk_wayland_surface_set_application_id(surface, application_id)&
& bind(c)
  use iso_c_binding, only: c_ptr, c_char
  type(c_ptr), value :: surface
  character(kind=c_char), dimension(*) :: application_id
...
function gdk_wayland_surface_inhibit_idle(surface) bind(c)
  use iso_c_binding, only: c_int, c_ptr
  integer(c_int) :: gdk_wayland_surface_inhibit_idle
  type(c_ptr), value :: surface
end function

! 
!void gdk_wayland_surface_uninhibit_idle (GdkSurface *surface);
subroutine gdk_wayland_surface_uninhibit_idle(surface) bind(c)
  use iso_c_binding, only: c_ptr
  type(c_ptr), value :: surface
end subroutine
...
! GDK_AVAILABLE_IN_ALL
!void gdk_wayland_display_set_cursor_theme (GdkDisplay *display, const char *theme, int size);
...
function gdk_x11_device_xi2_get_device_type(device) bind(c)
  use iso_c_binding, only: c_int, c_ptr
  integer(c_int) :: gdk_x11_device_xi2_get_device_type
  type(c_ptr), value :: device
end function

! 
!void gdk_x11_device_xi2_set_device_type (GdkX11DeviceXI2 *device, GdkX11DeviceType type);
subroutine gdk_x11_device_xi2_set_device_type(device, type) bind(c)
  use iso_c_binding, only: c_ptr, c_int
  type(c_ptr), value :: device
