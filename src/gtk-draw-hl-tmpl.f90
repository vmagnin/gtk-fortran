...
! Gtk modules for gtk-draw-hl-tmpl.f90
    ...
    use gtk, only: gtk_drawing_area_new, gtk_scrolled_window_add_with_viewport, &
        ...
        & CAIRO_FORMAT_RGB24, GDK_EXPOSURE_MASK, GDK_POINTER_MOTION_MASK, &
...
!!$GTK>=3.0!            & c_funloc(hl_gtk_drawing_area_expose_cb))
  ...
  function hl_gtk_drawing_area_get_gdk_pixbuf(area) &
    ...
    integer(kind=c_int) :: nx, ny, rs, rsp, fmt, isa
    integer :: lpix, lppix, nbp
    character(kind=c_char, len=1), dimension(:), pointer :: chdata, swdata
    integer :: i,j,n,m
    ...
    pixb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, isa, &
         & 8_c_int, nx, ny)
    rsp = gdk_pixbuf_get_rowstride(pixb)
    lppix = rsp*(ny-1) + nbp*nx

    ! N.B. We have to do this rather odd conversion because char* is usually
    ! a character string. Since the array is only ever accessed in C, we don't
    ! need to give it a real length.

    data = cairo_image_surface_get_data(surface)
    call c_f_pointer(data, chdata, [lpix])
    pdata = gdk_pixbuf_get_pixels(pixb)
    call c_f_pointer(pdata, swdata, [lppix])
    ...
    if (c_f_logical(isa)) then
       if (is_big_endian) then
          do i = 1, ny
             do j = 1, nx
                n = (i-1)*rs + 4*(j-1)+1
                m = (i-1)*rsp + 4*(j-1)+1
               swdata(m:m+3) = chdata(n+[1,2,3,0])
             end do
          end do
       else
          do i = 1, ny
