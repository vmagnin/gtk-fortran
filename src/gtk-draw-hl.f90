...
! --------------------------------------------------------
! gtk-draw-hl.f90
! Generated: Sat Jan 26 01:33:16 2013 GMT
...
!!$GTK< 3.0!
    pixb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, has_alpha, &
         & 8_c_int, xrange, yrange)
...
!!$GTK< 3.0!
    ...
    if (c_f_logical(has_alpha)) then
       if (is_big_endian) then
          do i = 1, yrange
             do j = 1, xrange
                n = (yorig+i-1)*rs + 4*(xorig+j-1)+1
                m = (i-1)*rsp + 4*(j-1)+1
