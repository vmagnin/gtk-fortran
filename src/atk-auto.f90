...
function atk_text_get_character_at_offset(text, offset) bind(c)
  use, intrinsic :: iso_c_binding, only: c_int32_t, c_ptr, c_int
...
function atk_add_global_event_listener(listener, event_type) bind(c)
  use, intrinsic :: iso_c_binding, only: c_int, c_funptr, c_char
...
subroutine atk_object_set_accessible_id(accessible, name) bind(c)
  use, intrinsic :: iso_c_binding, only: c_ptr, c_char
  type(c_ptr), value :: accessible
  character(kind=c_char), dimension(*) :: name
end subroutine
