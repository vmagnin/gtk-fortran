...
function pango_renderer_get_alpha(renderer, part) bind(c)
  use iso_c_binding, only: c_int16_t, c_ptr, c_int
...
function pango_config_key_get_system(key) bind(c)
  use iso_c_binding, only: c_ptr, c_char
  type(c_ptr) :: pango_config_key_get_system
...
!void pango_font_description_set_variations_static (PangoFontDescription *desc, const char *settings);
subroutine pango_font_description_set_variations_static(desc, settings) bind(c)
  use iso_c_binding, only: c_ptr, c_char
  type(c_ptr), value :: desc
  character(kind=c_char), dimension(*) :: settings
end subroutine

! PANGO_AVAILABLE_IN_1_42
!void pango_font_description_set_variations (PangoFontDescription *desc, const char *settings);
subroutine pango_font_description_set_variations(desc, settings) bind(c)
  use iso_c_binding, only: c_ptr, c_char
  type(c_ptr), value :: desc
  character(kind=c_char), dimension(*) :: settings
end subroutine
...
subroutine pango_get_log_attrs(text, length, level, language, log_attrs,&
& attrs_len) bind(c)
  use iso_c_binding, only: c_char, c_int, c_ptr
  character(kind=c_char), dimension(*) :: text
  integer(c_int), value :: length
...
function pango_fc_font_key_get_variations(key) bind(c)
  use iso_c_binding, only: c_ptr
  type(c_ptr) :: pango_fc_font_key_get_variations
  type(c_ptr), value :: key
end function
