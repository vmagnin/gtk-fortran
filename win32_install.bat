...
cmake -DGTK3_GLIBCONFIG_INCLUDE_DIR=%GLIBCONFIG_INCLUDE_DIR% -DPLPLOT_INCLUDE_DIR=%PLPLOT_INCLUDE_DIR% -DPLPLOT_LIBRARY=%PLPLOT_LIBRARY% -DPLPLOTF95_LIBRARY=%PLPLOTF95_LIBRARY% -DPLPLOTF95C_LIBRARY=%PLPLOTF95C_LIBRARY% -DPLPLOT_MODULE_DIR=%PLPLOT_MODULE_DIR% -DCMAKE_INSTALL_PREFIX=C:/gtk-3-fortran -G "MinGW Makefiles" ..

mingw32-make install

SET PKG_CONFIG_PATH=C:\gtk-3-fortran\lib\pkgconfig;%PKG_CONFIG_PATH%
pkg-config --cflags --libs gtk+-3.0 gtk-3-fortran > TMP.dat         
SET /p GTKFORTRAN_LIBS= < TMP.dat
DEL TMP.dat
SET GTKFORTRAN_LIBS=%GTKFORTRAN_LIBS% -LC:/gtk-3-fortran/lib
